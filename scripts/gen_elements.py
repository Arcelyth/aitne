...
def emit_map(elements) -> str:
    ...
    out.append("/// Generated by gen_elements.py")
    out.append("/// Edit ELEMENTS in this script, then rerun it to regenerate this file.")
    out.append("using @hashset {type HashSet}")
    out.append("")
    out.append("pub fn make_builtin_elements() -> HashSet[String] {")
    out.append("  let s = HashSet([])")
    for item in elements:
        out.append(f'  s.add("{item["tag"]}")')
    out.append("  s")
    out.append("}")
    out.append("")
    out.append("pub let builtin_elements : HashSet[String] = make_builtin_elements()")
    out.append("")
    return "\n".join(out).rstrip() + "\n"
